...
//	local _oAviso    := NIL
	...
	if len (_aPessoa) == 0
		...
		for _nUser = 1 to len (_aUsers)
			...
			if upper (left (_aUsers [_nUser, 3], 4)) != 'REP_' .and. ! upper (alltrim (_aUsers [_nUser, 3])) $ 'ADMINISTRADOR/CUPOM.CX/CUPOM.03/CUPOM.08/CUPOM.10/CUPOM.13/CUPOM LOJA 3/CUPOM SANDRA/CUPOM.TI/MANUTENCAO/SIGALOJA/4FX'
				...
				if ! PswSeek (_sIdUser, .T.)
					...
					// Onde buscar a 'pesso' do usuario: optei por informar no campo CARGO no sigacfg.
