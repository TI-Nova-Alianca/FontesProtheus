...
// 05/03/2024 - Robert  - Chamadas de metodos de ClsSQL() nao recebiam parametros.
//
...
User Function BatLojCGC()
	...
	_oSQL:Log ('[' + procname () + ']')
	_aCli := aclone (_oSQL:Qry2Array (.t., .f.))

	For _x:=1 to Len(_aCli)

		// Atualiza log a cada 100 registros para nao ficar muito lento
		if _x % 100 == 0
			U_Log2 ('debug', '[' + procname () + ']Verificando clientes (' + cvaltochar (_x) + ' de ' + cvaltochar (len (_aCli)) + ')')
		endif
		...
		//	u_log("Ajustado cupom:" + _aCli[_x, 2] +" Cliente:" + _aCli[_x, 3] +"-"+ alltrim(_aCli[_x, 6]) + "CGC:" + _aCli[_x, 5])
			U_Log2 ('info', '[' + procname () + "]Ajustado cupom:" + _aCli[_x, 2] +" Cliente:" + _aCli[_x, 3] +"-"+ alltrim(_aCli[_x, 6]) + "CGC:" + _aCli[_x, 5])
