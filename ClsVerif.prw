...
//			::Query +=                   " AND TIPO_FORNEC  != 'PROD_PROPRIA'"
	...
	case ::Numero == 10
		::Ativa     = .F.  // esse controle é feito atualmente pela agronomia através do NaWeb. Robert, 29/03/2023
		::Setores   = 'SAF'
		::ViaBatch  = .T.
		::GrupoPerg = "U_VALID005"
		::ValidPerg (_lDefault)
	...
	case ::Numero == 15
		::Ativa     = .F.  // Desde 2021 não usamos mais notas de entrada -> pré-nota -> nota de compra. Robert, 29/03/2023
		::Setores   = 'SAF'
		::ViaBatch  = .T.
		::GrupoPerg = "U_VALID005"
		::ValidPerg (_lDefault)
	...
	case ::Numero == 28
		...
		::Query := ""
		::Query += " WITH LISTA_OPS AS ("
		::Query +=    " SELECT DISTINCT D3_FILIAL, D3_OP"
		::Query +=    " FROM SD3010 SD3"
		::Query +=    " WHERE SD3.D_E_L_E_T_ = ''"
		::Query +=    " AND SD3.D3_EMISSAO BETWEEN '" + dtos (::Param01)  + "' AND '"+ dtos (::Param02) + "'"
		::Query +=    " AND SD3.D3_OP != ''"
		::Query +=    " AND SD3.D3_ESTORNO = ''"
		::Query +=    " AND SD3.D3_ORDEM = ''" // NAO QUERO O.S. DA MANUTENCAO
		::Query += "), C AS ("
		::Query += "SELECT SD3.D3_FILIAL AS FILIAL, SD3.D3_OP AS OP"
		::Query +=      ", SC2.C2_PRODUTO AS PROD_FINAL, RTRIM (PRODUZIDO.B1_DESC) AS DESCR_PROD_FINAL"
		::Query +=      ", dbo.VA_DTOC (MIN (D3_EMISSAO)) AS MENOR_DATA"
		::Query +=      ", dbo.VA_DTOC (MAX (D3_EMISSAO)) AS MAIOR_DATA"
	...
	case ::Numero == 48
		::Ativa      = .F.  // Nao temos previsao de voltar a operar com deposito fechado. Robert, 29/03/2023
		::Filiais    = '01'
		::Setores    = 'CUS'
		::ViaBatch  = .T.
	...
	case ::Numero == 50
		::Ativa      = .F.  // Atende de forma muito parcial. Ex: dessulfitados, 'antecede envase' e reprocessados acusariam sempre erro. Robert, 29/03/2023
		::Setores    = 'CUS'
		::ViaBatch  = .T.
	...
	case ::Numero == 56
		...
		::ValidPerg (_lDefault)
		::Descricao  = "Produto negativo no almox. retorno simbolico para matriz"
		::Sugestao   = ""
		::Query := ""
		::Query += " SELECT "
	...
	case ::Numero == 57
		::Ativa      = .F.  // Em tempos muito antigos, remetíamos para estocagem em terceiros controlando apenas poder de terceiros. O estoque era controlado via transferência para outro almoxarifado. Não usamos mais esse procedimento. Robert, 29/03/2023
	...
	case ::Numero == 59
		::Ativa      = .F.  // Jah desativado anteriormente. Em tempos muito antigos, remetíamos para estocagem em terceiros controlando apenas poder de terceiros. O estoque era controlado via transferência para outro almoxarifado. Não usamos mais esse procedimento. Robert, 29/03/2023
		::Setores    = 'CUS'
		::ViaBatch  = .T.
...
//			::Query +=    " AND FILIAL  = '" + cFilAnt + "'"
	...
	case ::Numero == 98
		...
		::Query +=     " and item_cod_item_log between ''" + iif (empty (::Param01), ' ', alltrim (::Param01)) + "'' and ''" + iif (empty (::Param02), ' ', alltrim (::Param02)) + "''"
		::Query +=     " and lote              between ''" + iif (empty (::Param03), ' ', alltrim (::Param03)) + "'' and ''" + iif (empty (::Param04), ' ', alltrim (::Param04)) + "''"
