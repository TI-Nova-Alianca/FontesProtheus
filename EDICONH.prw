...
// 15/05/2023 - Robert  - Rotina MATA103 nao deixa mais o SF1 posicionado no final (GLPI 13575)
//                      - Grava evento temporario apos importacao do CTe
//                      - Campo F1_VAFLAG passa a ser gravado junto na array aAutoCab
//                      - Desabilitado envio de e-mail ao coord. de logistica no final da importacao
//
...
Static Function _Importa(_xArq1, _XnomTransp1)
	...
	Do While _lContinua .and. !TRB->(Eof())
		...
		if _werro = 0
			...
			// inclui SF1
			_aAutoSF1 := {}
			// robert 15/05/23   AADD( _aAutoSF1, { "F1_FILIAL"   , xFilial('SF1')  , Nil } )
			...
			// robert 15/05/23  AADD( _aAutoSF1, { "F1_DTDIGIT"  , date()          , Nil } )
			AADD( _aAutoSF1, { "F1_VAFLAG"   , 'S'         , Nil } ) // Campo customizado, serah tratado pelo P.E. SF1100I - Flag Importacao
			...
			// robert 15/05/23   AADD(_aLinhas , {"D1_FILIAL"  , xFilial('SD1'        )      , Nil } )
			...
			DbSelectArea("SF1")
			Begin Transaction
				U_Log2 ('info', '[' + procname () + ']Importando CTe ' + _xNFiscal)
//				U_Log2 ('debug', '[' + procname () + ']Vou chamar MATA103. SF1 posicionado no recno ' + cvaltochar (sf1 -> (recno ())))
				MsExecAuto({|x,y,z|MATA103(x,y,z)},_aAutoSF1,_aAutoSD1,3)
//				U_Log2 ('debug', '[' + procname () + ']Voltou do MATA103. SF1 posicionado no recno ' + cvaltochar (sf1 -> (recno ())))
...
//					U_Log2 ('debug', '[' + procname () + ']SF1 -> (EOF ()) = ' + cvaltochar (SF1 -> (EOF ())))
					...
					_oEvento:Fornece    = _xFornece
					_oEvento:LojaFor    = _xLoja
					_oEvento:DiasValid  = 30
					_oEvento:ChaveNFe   = _xChave
					_oEvento:Grava ()

					// Campo vai ser passado junto na array do cabecalho.
					// // atualiza SF1
					// DbSelectArea("SF1")
					// RecLock("SF1",.F.)
				...
				Endif
			End Transaction
	//			U_Log2 ('debug', '[' + procname () + ']Fora da transacao.')
		else
