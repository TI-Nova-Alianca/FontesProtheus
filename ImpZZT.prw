...
// 20/01/2020 - Robert - Impr. de ticket de safra passa a ser completa, e somente em seguida imprime dados de portaria.
// 20/01/2020 - Robert - Novos parametros chamada geracao ticket safra. Melhorias para integracao com safra.
//
...
/*
            ...
            // Busca dados especificos de safra
            if !empty(ZZT->ZZT_CARGA) .and. !empty(ZZT->ZZT_SAFRA)
                ...
                if sze -> (dbseek (xfilial ("SZF") + ZZT->ZZT_SAFRA + ZZT->ZZT_CARGA, .F.))
                    if empty (zzt->zzt_pessai)  // 1a. pesagem
                        _sTicket += U_VA_RusTk (1,, 1, {}, .T., 'Bematech')
                    else  // 2a. pesagem
                        _sTicket += U_VA_RusTk (2,, 2, {}, .T., 'Bematech')
                    endif
                else
                    u_help ('Carga nao localizada para esta safra.')
                endif
            endif
