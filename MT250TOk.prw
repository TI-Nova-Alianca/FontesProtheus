...
user function mt250tok ()
	...
	if _lRet .and. empty (m->d3_vaturno)
		u_help ('Turno de producao deve ser informado.')
		_lRet = .F.
	endif
	...
	if _lRet .and. m->d3_tm != '010'
		u_help ("Apontamento de producao deveria usar tipo de movimento 010.")
		_lRet = .F.
	endif
	...
	if _lRet .and. (m->d3_qtganho > 0 .or. m->d3_qtmaior > 0) .and. fBuscaCpo ("SB1", 1, xfilial ("SB1") + m->d3_cod, "B1_TIPO") == 'PA'  
		u_help ("Para produto acabado nao deve ser produzida quantidade acima do previsto na OP.")
		_lRet = .F.
	endif
...
static function _VerEtiq ()
	...
	if ! empty (m->d3_vaetiq)
		...
		if ! za1 -> (dbseek (xfilial ("ZA1") + m->d3_vaetiq, .F.))
			u_help ("Etiqueta '" + m->d3_vaetiq + "' nao localizada.")
			_lRet = .F.
		endif
		if _lRet .and. (m->d3_quant + m->d3_perda) != za1 -> za1_quant
			u_help ("Quantidade produzida + perdida nao pode ser diferente da quantidade da etiqueta.")
			_lRet = .F.
		endif
		if _lRet .and. m->d3_op != za1 -> za1_op
			u_help ("O.P. nao pode ser diferente da O.P. da etiqueta.")
			_lRet = .F.
		endif
		if _lRet .and. m->d3_cod != za1 -> za1_prod
			u_help ("Produto nao pode ser diferente do produto da etiqueta.")
			_lRet = .F.
		endif
		if _lRet .and. m->d3_parctot != 'P'
			u_help ("Quando informada etiqueta, a producao deve ser sempre 'Parcial'. Para encerrar a OP use opcao 'Encerrar'.")
			_lRet = .F.
		endif
	endif
return _lRet
...
static function _VerData ()
    ...
    if _lRet
		if m->d3_emissao > date ()
			u_help ("Data de apontamento nao pode ser maior que a data atual.")
			_lRet = .F.
		endif
	endif
	...
	if _lRet
		...
		if sc2 -> (dbseek (xfilial ("SC2") + m->d3_op, .F.))
			if _lRet .and. m->d3_emissao < sc2 -> c2_emissao
				u_help ("Data do movimento nao pode ser menor que a data de emissao da OP (" + dtoc (sc2 -> c2_emissao) + ").")
				_lRet = .F.
			endif
			...
			if _lRet .and. left (dtos (m->d3_emissao), 6) > left (dtos (sc2 -> c2_datprf), 6)
				u_help ("Data do movimento nao pode estar em mes posterior da data prevista de termino da OP (" + dtoc (sc2 -> c2_datprf) + ").")
				_lRet = .F.
			endif
			if _lRet .and. m->d3_emissao != sc2 -> c2_datprf
				_lRet = U_MsgNoYes ("Apontamento em data diferente da data prevista de termino da OP (" + dtoc (sc2 -> c2_datprf) + "). Confirma assim mesmo?")
			endif
		endif
	endif
...
static function _VerFull ()
	...
	if _lRet .and. ! empty (_sAlmFull) .and. fBuscaCpo ("SB1", 1, xfilial ("SB1") + m->d3_cod, 'B1_VAFULLW') == 'S'
		if m->d3_local != _sAlmFull
			u_help ("Produto controlado pelo Fullsoft: OP deve ser apontada no almoxarifado de integracao (" + _sAlmFull + ").")
			_lRet = .F.
		endif
		if empty (m->d3_vaetiq)
			u_help ("Produto controlado pelo Fullsoft: deve ser informado numero da etiqueta.")
			_lRet = .F.
		endif
	endif
return _lRet
...
static function _VerEmpenh ()
	...
	if _lRet
		sc2 -> (dbsetorder (1))
		if ! sc2 -> (dbseek (xfilial ("SC2") + m->d3_op, .F.))
			u_help ("OP nao encontrada no cadastro!")
			_lRet = .F.
		endif
	endif
	...
	if _lRet .and. sc2 -> c2_vaopesp == 'R'  // Reprocesso
		...
		if _oSQL:RetQry () > 0
			u_help ("OP de retrabalho nao deve ter empenhos. Os materiais devem ser requisitados manualmente.")
			_lRet = .F.
		endif
	endif
	...
	if _lRet .and. sc2 -> c2_vaopesp == 'E'  // OP externa
		...
		if ! empty (_oSQL:Qry2Str (1, ', '))
			u_help ("OP do tipo externa: O(s) seguinte(s) materiais constam nos empenhos da OP, mas ja tiveram requisicao via NF. Elimine os empenhos ou exclua a NF: " + _oSQL:_xRetQry)
			_lRet = .F.
		endif
	endif
	...
	if _lRet .and. ! sc2 -> c2_vaopesp $ 'E/R'
		...
		_oSQL:_sQuery +=   " AND SD4.D4_QUANT   > 0"
		if _oSQL:RetQry () == 0
			u_Help ("Esta OP nao tem empenhos, ou ja foram zerados.")
			_lRet = .F.
		endif
	endif
	...
	if _lRet
		...
		if _oSQL:RetQry () > 0
			u_Help ("Encontrei empenho do item '" + fBuscaCpo ("SC2", 1, xfilial ("SC2") + m->d3_op, "C2_PRODUTO") + "' (mesmo item a ser produzido pela OP). Remova esse empenho antes de apontar a OP para evitar recursividade.")
			_lRet = .F.
		endif
	endif
return _lRet
...
//	u_logIni ()
	...
	if _lRet .and. _lEhReproc
		if _lRet .and. m->d3_quant > 0
			u_help ("OP de retrabalho / reprocesso: a quantidade deve ser informada como 'perda'.")
			_lRet = .F.
		endif
	endif
	...
	if _lRet .and. _lEhReproc
		...
		if ! sb2 -> (dbseek (xfilial ("SB2") + m->d3_cod + _sAlmRetr, .F.))
			u_help ("O produto retrabalhado (" + alltrim (m->d3_cod) + ") deve estar disponivel no almoxarifado '" + _sAlmRetr + "', pois o apontamento de uma OP de retrabalho precisa gerar transferencia para o almoxarifado de integracao com o FullWMS, e neste caso nao ha de onde fazer essa transferencia.")
			_lRet = .F.
		else
			_nSalDisp = sb2 -> b2_qatu - sb2 -> b2_reserva - sb2 -> b2_qaclass
			if m->d3_perda > _nSalDisp
				u_help ("Saldo disponivel (" + cvaltochar (_nSalDisp) + ") do produto '" + alltrim (m->d3_cod) + "' no almoxarifado '" + _sAlmRetr + "' deve ser no minimo " + cvaltochar (m->d3_perda) + ", pois essa quantidade precisa ser transferida para o almoxarifado de integracao com FullWMS para entrada na expedicao.")
				_lRet = .F.
			endif
		endif
	endif
