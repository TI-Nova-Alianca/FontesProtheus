...
// 26/03/2024 - Robert - Removido campo A2_VANRDAP (teria que ler da ClsAssoc, mas nunca mais foi solicitado).
//
...
//	_oSQL:_sQuery += " RTRIM (FILIAL) AS FILIAL,NF_ENTRADA,dbo.VA_DTOC (EMISSAO) AS EMISSAO,TIPO_UVA,COR_UVA,RTRIM (PRODUTO) AS PRODUTO, RTRIM (DESCRICAO) AS DESCRICAO,GRAU,CLASSIFICACAO,"
	...
	_oSQL:_sQuery += " '''' + F1_CHVNFE AS CHAVE_NFE"
...
//	_oSQL:_sQuery += " GROUP BY NOME_ASSOCIADO,CPF,FILIAL,PRODUTO,DESCRICAO,TIPO_UVA,COR_UVA,GRAU,MUNICIPIO,NF_ENTRADA,EMISSAO,F1_CHVNFE,A2_COD_MUN, CLASSIFICACAO"
	_oSQL:_sQuery += " GROUP BY SAFRA, TIPO_NF, NOME_ASSOCIADO,CPF,FILIAL,PRODUTO,DESCRICAO,TIPO_UVA,COR_UVA,GRAU,MUNICIPIO,NF,EMISSAO,F1_CHVNFE,A2_COD_MUN, CLASSIFICACAO"
	_oSQL:Log ()
	_oSQL:ArqDestXLS = 'VA_XLS42'
	_oSQL:Qry2Xls ()
return
