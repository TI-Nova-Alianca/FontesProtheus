...
// --------------------------------------------------------------------------
User Function ZX5_43 ()
	U_ZX5A (4, "43", "U_ZX5_43LO ()", "allwaystrue ()")
return

// --------------------------------------------------------------------------
// Linha OK
User Function ZX5_43LO ()
	local _lRet := .T.
	local _oSQL := NIL

	// Verifica linha duplicada.
	if _lRet .and. ! GDDeleted ()
		_lRet = GDCheckKey ({"ZX5_43COD"}, 4)
	endif
	...
	if _lRet .and. GDDeleted ()
		...
		_oSQL:_sQuery += " AND N1_VAZX543 = '" + GDFieldGet ("ZX5_43COD") + "'"
		if _oSQL:RetQry () > 0
