...
// --------------------------------------------------------------------------
User Function ZX5_44 ()
	U_ZX5A (4, "44", "U_ZX5_44LO ()", "allwaystrue ()")
return

// --------------------------------------------------------------------------
// Linha OK
User Function ZX5_44LO ()
	local _lRet := .T.
	local _oSQL := NIL

	// Verifica linha duplicada.
	if _lRet .and. ! GDDeleted ()
		_lRet = GDCheckKey ({"ZX5_44COD"}, 4)
	endif
	...
	if _lRet .and. GDDeleted ()
		...
		_oSQL:_sQuery += " AND N1_VAZX544 = '" + GDFieldGet ("ZX5_44COD") + "'"
		if _oSQL:RetQry () > 0
