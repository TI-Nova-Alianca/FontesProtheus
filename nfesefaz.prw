...
// Autor....: Eduardo Riera
// Data.....: 13/07/2007
// Descrição: Rdmake de- exemplo para geracao da Nota Fiscal Eletronica do SEFAZ - Versao T01.00 / 2.00                                ³±±
//
// Historico de alteracoes:
// 08/09/2008 - Robert  - Busca dados adicionais e insc.ST na tabela ZZ4
// 07/10/2008 - Robert  - Desconta F1_CONTSOC (FunRural) do total da nota.
//                      - Nao busca mais UM e quant. do SB5 (usava fator de conversao, etc).
//                      - Acrescenta '*' ao numero da duplicata, quando for de subst.tributaria.
// 08/10/2008 - Robert  - Soh envia dados de volumes se tiver volume e especie preenchidos.
//                      - tags de volume e especie estavam invertidas no XML.
//                      - Enviava 2 campos do CD2 a menos nas notas de entrada.
// 11/11/2008 - Robert  - Envia D2_PRCVEN em lugar do D2_PRUNIT
// 28/11/2008 - Robert  - Somava D2_DESCON ao D2_TOTAL no total do item da nota.
// 01/12/2008 - Robert  - Quando NF tipo D/B nao invertia indices e campos na leitura do CD2.
// 05/12/2008 - Robert  - Somava D1_VALDESC ao D1_TOTAL no total do item da nota.
//                      - Em notas de saida tipo C,I,P buscava dados das notas originais para remontar (errada) a chave das NFe originais.
//                        foi alterado para simplesmente buscar a chave jah gravada no SF2 quando enviou a NF original.
// 12/12/2008 - Robert  - Tratamento para notas de entrada de devol. de venda (valor sem desconto)
//                      - Em notas de entrada tipo D (devol. de venda feita com NFe), busca a chave jah gravada no SF2 quando enviou a NF original.
//                      - Array aIcmsST tinha 2 campos a menos quando gerada para nota de entrada.
// 06/01/2009 - Robert  - Leitura de valores de ISS tentava buscar no SD2 para notas de entrada.
//                      - Busca descricao do produto do D1_DESCRI (quando disponivel) e nao mais do B1_DESC para as notas de entrada.
//                      - Aumentada descricao do produto para 120 posicoes (maximo cfe. schema XML) para NF entrada.
//                      - Busca F1_VALBRUT para o total da NF quando for NF de importacao.
// 07/01/2009 - Robert  - Envio de especies (D1_vaVolEs) e quantidades (D1_vaVolQt) de volumes (campos customizados).
// 20/01/2009 - Robert  - Buscava o texto apenas do primeiro TES para a nat. de operacao.
// 28/01/2009 - Robert  - Busca campo F4_VATEXTO, se preenchido, em lugar do F4_TEXTO.
// 17/02/2009 - Robert  - Monta array aICMS com dados do SD1, caso nao encontre no CD2.
// 12/03/2009 - Robert  - Exportava F2_PLIQUI e F2_PBRUTO em todas as especies de volumes. Agora exporta apenas na primeira.
// 13/03/2009 - Robert  - Envia insc.ST tambem para nota de entrada (busca do ZZ4).
// 26/03/2009 - Robert  - Incluido D1_BRICMS na montagem de aICMSST quando nao encontra no CD2.
// 18/08/2009 - Robert  - Busca e-mail no campo A1_VAMDANF.
// 03/09/2009 - Robert  - Tratamento para o campos D1_VAFOROR e D2_VACLIOR.
//                      - Criada funcao _NFeID para uso em NFs que referenciam alguma NF-e de entrada.
// 22/09/2009 - Robert  - Ajustes para incompatibilidade de tamanhos entre D2_ITEM e D2_ITEMORI nas NF de compl. ICMS.
// 03/11/2009 - Robert  - Schema nao aceita mais de uma NF referenciada.
// 21/12/2009 - Robert  - Buscava mensagens do SC5 desnecessariamente (jah constavam no ZZ4).
// 08/02/2010 - Robert  - Busca e-mail do fornecedor no campo A2_VAMDANF.
// 16/02/2010 - Robert  - Alteracoes diversas para acompanhar melhorias da versao padrao.
//                      - Inclusao de customizacoes para notas de importacao.
// 03/03/2010 - Robert  - Criada funcao _AchaNFOri (por enquanto somente para as notas de entrada).
// 02/06/2010 - Robert  - Ajustes em tags especificas para NF de importacao.
// 04/08/2010 - Robert  - Monta array aICMS com dados do SD2, caso nao encontre no CD2.
// 26/08/2010 - Robert  - Ajustes para NF de importacao.
// 24/11/2010 - Robert  - Envia copia do XML para a transportadora.
// 08/02/2011 - Robert  - Quando nao encontrava PIS e COFINS no arquivo CD2, mandava CST='08' nas suas tags. Agora manda '01'.
// 31/03/2011 - Robert  - Ajustes para versao 2.0 da NF-e.
// 10/04/2011 - Robert  - Ajustes para versao 2.0 - NF de importacao.
// 27/04/2011 - Robert  - Campos A1_PAIS e A2_PAIS nao recebem mais tratamento default para assumir 1058. Agora devem estar preenchidos.
//                      - Desabilitada copia do e-mail para o A4_MAIL (padrao), pois para isso temos o A4_VAMDANF.
// 01/06/2011 - Robert  - Adicionados dados para 'adicao' nas notas de importacao.
// 07/06/2011 - Robert  - Desabilitada tag 'motDesICMS' pois gerava duplicidade do elemento 'vBC'.
// 17/06/2011 - Robert  - Envia o campo E1_VANCTO e nao mais o E1_VENCORI nas duplicatas.
// 13/10/2011 - Robert  - Leitura do ZZ4 passa a usar a funcao VA_ZZ4L.
// 26/10/2011 - Robert  - Adequacoes para versao 2.01 (entra em producao dia 26/10 ao meio-dia).
// 27/11/2011 - Robert  - Passa a buscar dados de importacao da tabela CD5 e nao mais de campos customizados do SD1.
// 21/12/2011 - Robert  - Gerava 2 linhas na array aCST em notas de saida (devo ter deixado a versao original sem comentariar).
// 06/02/2012 - Robert  - Passa a enviar tags de NF de produtor referenciada (funcao _AchaNFPro).
// 16/04/2012 - Robert  - Adequacoes p/ somar IPI de NF devol.compras em 'outros' cfe. fonte Totvs de 10/04/2012.
// 09/05/2012 - Robert  - Quando nao encontrava PIS e COFINS no arquivo CD2, mandava CST='01' nas suas tags. Agora nao gera mais as tags. Ver obs. do dia 08/02/2011.
// 29/06/2012 - Robert  - Adequacoes diversas para ficar dentro do padrao da Totvs, tanto quanto possivel.
// 28/09/2012 - Robert  - Ajustes para gerar tags de PIS e COFINS pelo SFT quando nao encontrar nada no CD2.
// 17/10/2012 - Robert  - Desabilitada funcao _NfeID pois agora temos a chave da NF gravada no SF1.
// 23/10/2012 - Robert  - Adequacoes para ficar dentro do padrao Microsiga.
// 05/06/2013 - Robert  - Adequacoes para ficar dentro do padrao Microsiga (lei transparencia fiscal).
// 07/06/2013 - Leandro - Ajuste para que a lei da transparencia fiscal seja valida apenas para consumidor final
// 06/08/2013 - Robert  - Nao desconta D1_II no total do item
//                      - Soma D1_II no total da nota.
// 05/08/2014 - Robert  - Acrescenta lote (quando tiver) `a descricao do produto.
// 29/09/2014 - Robert  - Nao acrescenta mais o lote `a descricao do produto, pois vamos aglutinar os lotes do pedido.
// 23/02/2015 - Catia   - Versão 3.1 NFE
// 28/02/2015 - Robert  - Versão 3.1 NFE
// 06/03/2015 - Catia   - Alterado a atribuição do tipo de nota - procurar por '<tpNFe>'+cTPNota+'</tpNFe>'
// 31/03/2015 - Catia   - situacao nao prevista complemento de preço gerar  cTPNota:= "2"
// 30/04/2015 - Robert  - Verifica serie da nota antes de gerar o XML.
// 27/06/2015 - Robert  - Passa a gerar tag <IEST> pelo padrao do sistema.
// 13/07/2015 - Catia   - Nao estava prevista finalidade 3 para notas de complemento de IPI - notas tipo 'P'
// 17/08/2015 - Robert  - Campo F1_HORA aumentado de 5 para 8 posicoes na release R9.
// 21/08/2015 - Robert  - Desabilitada concatenacao do e-mail da transportadora, para testes (ver coment. no local)
// 26/08/2015 - Robert  - Habilitada novamente concatenacao do e-mail da transportadora (erro envio era de autenticacao no servidor de e-mail).
// 24/09/2015 - Robert  - Ajustada funcao _GetNFEIMP para retornar mais dados (versao 3.10)
// 28/10/2015 - Robert  - Leitura de mensagens do ZZ4 desabilitada para notas de saida (jah fazia leitura posterior do f2_vacmemc e f2_vacmemf)
//                      - Habilitada leitura de mensagens do f1_vacmemc e f1_vacmemf e posterior log comparando com o ZZ4 para ver se posso desabilitar o ZZ4.
//                      - Geracao da tag <tpNFe> revista para ficar de acordo com o fonte padrao da Totvs.
//                      - Tratamento na geracao da tag <tpNFe> para notas de complemento de preco de uva (tipo C + aNFVincRur)
// 29/12/2015 - Robert  - Verifica se o campo F2_TRANSP estah preenchido.
// 05/01/2016 - Robert  - Liberada serie 30 (notas de safra).
//                      - Nao busca mais os dados adicionais do ZZ4. Busca de campos memo amarrados ao SF1 e SF2.
// 07/01/2016 - Robert  - Atualizacoes diversas cfe. fonte padrao, para novas legislacoes de 2016.
// 12/01/2016 - Robert  - Volta a buscar descricao do D1_DESCRI e nao mais do B1_DESC.
//                      - Incluido codigo do cliente/fornecedor no final do nome do mesmo.
// 04/03/2016 - Robert  - Funcao _AchaNFOri substituida pela versao padrao.
// 30/04/2016 - Robert  - Adequacao ao fonte padrao da Totvs (implantacao do DIFAL).
// 05/05/2016 - Robert  - Volta a zerar desconto (indevidamente desabilitado em 30/04/2016).
// 19/12/2016 - Robert  - Chamada de busca da NF de produtor passa a ser executada independente do campo D1_NFORI estar preenchido.
// 05/01/2016 - Catia   - tag ICMSUFDest - não estava sendo gerada - havia um teste com a data o ano de emissao = "2016"  
// 11/04/2018 - Catia   - NFESEFAZ TOVS - 02/04/2018
// 27/07/2018 - Catia   - Alterado para colocar o codigo do fornecedor ao lado do nome - igual aos clientes
// 03/09/2018 - Robert  - Atualizada function NfeCob conforme pacote Totvs (SEFAZ aumentou validacoes tags de cobranca).
// 11/09/2018 - Catia   - Teste da serie que pode ser transmitida incluidas as series 001/002 e 007
// 17/10/2018 - Robert  - Se tiver transportadora de redespacho (F2_REDESP), inclui nos destinatarios de e-mail.
// 13/11/2018 - Catia   - tratamento campo EMAIL para o cliente/fornecedor usando como prioridade o campo Ax_VAMDANF e na falta dele o campo Ax_EMAIL
// 29/11/2018 - Catia   - alterado no fonte padrao para que NAO envie o email pro transportador
// 08/01/2019 - Catia   - alterações da difal participativa - conforme comparacao fonte padrao
// 23/05/2019 - Catia   - alterações para a NT005.2018 - novas tags
// 26/06/2019 - Catia   - modalidade de frete não estava ok
// 26/07/2019 - Robert  - Volta a usar tratamento padrao do Protheus para codigos de barras (nao usa mais B1_vaEanUn)
// 28/08/2019 - Robert  - Baixado fonte padrao e reaplicadas as customizacoes.
// 03/09/2019 - Robert  - Baixado fonte padrao e reaplicadas as customizacoes; habilitado envio de e-mail para transp. e transp.redesp.
...
// 25/06/2024 - Sandra  - Incluido trecho customizado. GLPI: 15646
//
// --------------------------------------------------------------------------
#INCLUDE "PROTHEUS.CH"   
#INCLUDE "COLORS.CH"
...
// customização
// Alianca: temos algumas series de notas que nao devem se transmitidas, pois sao usadas apenas para amarracao de operacao triangular no sistema, etc.
if ! cSerie $ '10 /20 /30 /001/002/007'
	u_help ("A serie '" + cSerie + "' nao deve ser transmitida para a SEFAZ.")
	return
endif
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				// Alianca: codigo junto ao nome do cliente //customização
				aadd(aDest,alltrim (SA1->A1_NOME) + ' - ' + sa1 -> a1_cod)
				...
				// Alianca: campo customizado para e-mail de envio do DANFe
				aadd(aDest,iif (empty (SA1 -> A1_VAMDANF), SA1->A1_EMAIL, SA1 -> A1_VAMDANF))
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					...
					aadd(aDest,AllTrim(SA1->A1_CGC))
					// Alianca: codigo junto ao nome do cliente
					aadd(aDest,alltrim (SA1->A1_NOME) + ' - ' + sa1->a1_cod)
					...
					// Alianca: nada de assumir o pais por default. Queremos o que consta no cadastro do cliente.
					...
					if ! Empty (SA1->A1_PAIS)
						aadd(aDest,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP"))
						aadd(aDest,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" ))
					else
						aadd(aDest,'')
						aadd(aDest,'')
					endif
					...
					// Alianca: campo customizado para e-mail do cliente
					aadd(aDest,iif (empty (sa1 -> a1_vamdanf), SA1->A1_EMAIL, sa1 -> a1_vamdanf))
				...
				Else
					...
					// Alianca: codigo junto ao nome
					aadd(aDest,SA2->A2_NOME + SA2->A2_COD)
					...
					// Alianca: nada de assumir o pais por default. Queremos o que consta no cadastro do fornecedor.
					...
					if ! Empty(SA2->A2_PAIS)
						aadd(aDest,Posicione("SYA",1,xFilial("SYA")+SA2->A2_PAIS,"YA_SISEXP"))
						aadd(aDest,Posicione("SYA",1,xFilial("SYA")+SA2->A2_PAIS,"YA_DESCR"))
					else
						aadd(aDest,'')
						aadd(aDest,'')
					endif
					...
					// Alianca: campo customizado para envio por e-mail
					aadd(aDest,iif (empty (SA2 -> A2_VAMDANF), SA2->A2_EMAIL, SA2 -> A2_VAMDANF))		
				...
				If !Empty(SF2->F2_TRANSP)
					...
					EndIf
				EndIf
				// Alianca: Se tiver endereco de e-mail informado na transportadora, usa-o para enviar uma copia do XML.
				if ! empty (sa4 -> a4_vaMDanf) .and. len (aDest) >= 16
					aDest [16] = alltrim (aDest [16]) + iif (!empty (aDest [16]),";","") + alltrim (sa4 -> a4_vamdanf)
				EndIf
				//
				// Alianca: se tiver transportadora de redespacho, inclui nos destinatarios.
				if ! empty (sf2 -> f2_redesp)
					_sMailRed = fBuscaCpo ("SA4", 1, xfilial ("SA4") + sf2 -> f2_redesp, "A4_VAMDANF")
					if ! empty (_sMailRed) .and. len (aDest) >= 16
						u_log ('adicionando email de redespacho:', _sMailRed)
						aDest [16] = alltrim (aDest [16]) + iif (!empty (aDest [16]),";","") + alltrim (_sMailRed)
					endif
				EndIf
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If lContinua
						...
						// Alianca: Monta texto da natureza de operacao buscando campo customizado, caso preenchido.
						_sTxtNatOp = iif (empty (sf4 -> f4_vaTexto), alltrim (SF4->F4_TEXTO), alltrim (sf4 -> f4_vaTexto))
						if ! _sTxtNatOp $ cNatOper
							if ! empty (cNatOper)
								cNatOper += ";"
							Endif
							cNatOper += _sTxtNatOp
						...
						// Alianca: Quero buscar exclusivamente da nota fiscal. Pra isso é que tem os campos lah, certo?
						//If Empty(aVeiculo)
							//DbSelectArea("DA3")
							//DbSetOrder(1)
						...
						*/
						...
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							EndIf
			            Else 
							// Aliança: customização exportação
							nDesconto := 0
						...
						*/
						...
						lDeduzDeson := (SFT->FT_ISENICM > 0 .and. !Empty(SFT->FT_MOTICMS) .and. (SFT->FT_DESCICM > 0 .and. SFT->FT_AGREG == 'D')) .or. (Alltrim(SFT->FT_MOTICMS) == '7' .and. !Empty(cICMSZFM))
		...
		Else				
				...
				*/
				...
				Else
					...
					*/
					...
					// Alianca: nao queremos que assuma o pais por default.
					//aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP")))
					//aadd(aDest,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_DESCR" )))
					if ! Empty(SA1->A1_PAIS)
						aadd(aDest,Posicione("SYA",1,xFilial("SYA")+SA1->A1_PAIS,"YA_SISEXP"))
					...
					EndIf
					aadd(aDest,SA1->A1_SUFRAMA)
					// Alianca: campo customizado para e-mail
					aadd(aDest,iif (empty (sa1 -> a1_vamdanf), SA1->A1_EMAIL, sa1 -> a1_vamdanf))
					aAdd(aDest, SA1->A1_CONTRIB) // Posição 17
					aadd(aDest,Iif(SA1->(FieldPos("A1_IENCONT")) > 0 ,SA1->A1_IENCONT,""))
					aadd(aDest,SA1->A1_INSCRM)
			...
			Else
				...
				Else
					...
					aadd(aDest,AllTrim(SA2->A2_CGC))
					// Alianca: codigo junto ao nome
					aadd(aDest,alltrim (SA2->A2_NOME) + ' - ' + sa2 -> a2_cod)
					aadd(aDest,MyGetEnd(SA2->A2_END,"SA2")[1])
			...
			// Alianca: Busca mensagens adicionais de interesse do fisco e do contribuinte.
			cMensFis = MSMM (sf1->f1_vacmemf,,,,3)
			cMensCli = MSMM (sf1->f1_vacmemc,,,,3)
			//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
			//³Posiciona transportador                                                 ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//SOLUÇÃO PALIATIVA REFERENTE A ISSUE DSERTSS1-23228 REMOVER O TRECHO QUE ENVOLVE A VARIAVEL lObsFisc MANTER SÓ O CONTEUDO DO ELSE E OS COLUMNPOS.
				lObsFisc := SuperGetMV("MV_OBSFISC", .F., .F.)
				...
				If SD1->(ColumnPos("D1_OBSFISC")) <> 0 .And. SD1->(ColumnPos("D1_OBSCONT")) <> 0 .And. SD1->(ColumnPos("D1_OBSCTIT")) <> 0 .And. SD1->(ColumnPos("D1_OBSFTIT")) <> 0
					If lObsFisc
						If !Empty((cAliasSD1)->D1_OBSCONT) .Or. !Empty((cAliasSD1)->D1_OBSFISC)
							aAdd(aObsFisco, { Alltrim((cAliasSD1)->D1_OBSFTIT), AllTrim((cAliasSD1)->D1_OBSFISC) })
						EndIf
					Else
						aObsItem := {}
						If !Empty((cAliasSD1)->D1_OBSCONT) .Or. !Empty((cAliasSD1)->D1_OBSFISC)
				...
				*/
				...
				// Alianca: Inicio bloco customizado para buscar NF de produtor rural
				if sf1 -> f1_formul == 'S' .and. ! empty (sf1 -> f1_vanfpro)
					//u_log ('Vou procurar NF produtor:', sf1 -> f1_vanfpro)
					_AchaNFPro (cAliasSD1, @aNFVincRur)
				endif
				// Alianca: Fim bloco customizado para buscar NF de produtor rural
				If lVinc .and. !Empty(aNfVinc)
					aADD(aItemVinc,{ATail(aNfVinc)[1]})
				Else
					aADD(aItemVinc,{})
		       ...
		       EndIf   		
				...
				// Alianca: queremos a descricao do SD1
				cDescProd := iif (empty ((cAliasSD1)->D1_DESCRI), sb1 -> b1_desc, (cAliasSD1)->D1_DESCRI)
			...
			//	U_Log2 ('debug', '[' + procname () + ']lChvCdd depois: ' + cvaltochar (lChvCdd))
				...
				*/
				...
				If lEasy  .And. ( !Empty((cAliasSD1)->D1_TIPO_NF) .Or. ( Empty((cAliasSD1)->D1_TIPO_NF) .And. (cAliasSD1)->D1_TIPO $ 'IPC' )   )
					...
					If !cTipoNF$"IPC" .And. cTipoNFEnt <> "6"
						...
						//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
						// Alianca: nao temos o modulo de importacao --> aadd(aDI,(GetNFEIMP(.F.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItemPC)))
						aadd(aDI,_GetNFEIMP(.F.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItemPC, cTipoNF, (cAliasSD1) -> d1_cod, (cAliasSD1) -> d1_item))						
					Else
						//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
						...
						EndIf
						//Alianca: nao temos o modulo de importacao --> aadd(aDI,(GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,cItemOri)))
						aadd(aDI,_GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,alltrim(Str(VAL(cItemOri))), cTipoNF))
					EndIf
					aAdi := aDI
				...
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "NDCPI")
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
							aAdd(aDI,{;
								...
								{"I23d","CNPJ", IIf( CD5->(FieldPos("CD5_CNPJAE"))>0 .and. !empty(CD5->CD5_CNPJAE),CD5->CD5_CNPJAE, iif(isNT230004() .and. CD5->(FieldPos("CD5_CPFAE"))>0,CD5->CD5_CPFAE,"")) },;			//39
				...
				*/
				...
				lSomaPISST	  := .F.
                lSomaCOFINSST := .F.
				// Alianca: Monta array com as especies de volumes, contendo volume, peso liquido e bruto
				// de cada especie (campos customizados).
				_nEspVol = ascan (aEspVol, {|_aVal| _aVal [1] == (cAliasSD1)->D1_vaVolEs})
				if _nEspVol == 0
					aadd (aEspVol, {(cAliasSD1)->D1_vaVolEs, 0, 0, 0})
					_nEspVol = len (aEspVol)
				endif
				aEspVol [_nEspVol, 2] += (cAliasSD1)->D1_vaVolQt
				aEspVol [_nEspVol, 3] += (cAliasSD1)->D1_pesbrt - (cAliasSD1)->D1_tara
				aEspVol [_nEspVol, 4] += (cAliasSD1)->D1_pesbrt
...
IF lPe01Nfe     
	// Alianca: Deixar private para ser vista pelo ponto de entrada PE01NFESEFAZ    
	private _sNFEntSai := cTipo  
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		...
		cString += 	NfeItem(aProd[nX]		,aICMS[nX]		,aICMSST[nX]	,aIPI[nX]	,aPIS[nX]		,aPISST[nX]		,aCOFINS[nX]	,aCOFINSST[nX]	,aISSQN[nX]		,aCST[nX]		,;
							...
							@lMonof61		,aICMSMono[nX]	,aBenef[nX]		,lDeduzDeson,aCredPresum[nX])
	Next nX
...
Static Function NfeItem(aProd		, aICMS			, aICMSST	, aIPI			, aPIS	   		, aPISST	 , aCOFINS		, aCOFINSST		, aISSQN		, aCST			,;
						...
						lMonof61	, aICMSMono 	, aBenef	, lDeduzDeson	, aCredPresum	)
...
If Len(aObsFisco) > 0
	cString += '<obsFisco>'
	cString += '<xCampo>'+Substr(aObsFisco[1][1], 1, 20)+ '</xCampo>'
	cString += '<xTexto>'+Substr(aObsFisco[1][2], 1, 60)+ '</xTexto>'
	cString += '</obsFisco>'	
EndIf
...
IF len(aDetPag) > 0  
	...
	For nX := 1 To Len(aDetPag)
		...
		if isNT230004() .and. Len(aDetPag[nX]) > 9 .and. !empty(aDetPag[nX][10])
			cString += '<dPag>'+ ConvType(aDetPag[nX][10]) +'</dPag>'
		endIf

		if isNT230004() .and. Len(aDetPag[nX]) > 10 .and. len(aDetPag[nX][11]) = 2 .and. !empty(aDetPag[nX][11][1])
			cString += 	'<CNPJPag>'+ aDetPag[nX][11][1] +'</CNPJPag>'
			cString += 	'<UFPag>'+ aDetPag[nX][11][2] +'</UFPag>'
		endIf

		If Len(aDetPag[nX]) > 3 
			IF	!empty(aDetPag[nX][4]) .and. aDetPag[nX][1] $ '03-04' + Iif(isNT230004(),"17-18","") 
				cString += '<cartoes>'
				...
				if isNT230004() .and. Len(aDetPag[nX]) > 11 .and. !empty(aDetPag[nX][12])
					cString += '<CNPJReceb>'+ aDetPag[nX][12] +'</CNPJReceb>'
				endIf

				if isNT230004() .and. Len(aDetPag[nX]) > 12 .and. !empty(aDetPag[nX][13])
	...
	If !Empty(cCondPag)
		Do Case
			...
			Case cCondPag == "CD"//CARTAO DE DEBITO AUTOMATICO
				cForma := "04"
			Case (!LjIT240002() .and. cCondPag == "CR") .or. (cCondPag == "CLJ") //CREDITO LOJA ou //Cartão da Loja (Private Label)
				cForma := "05"
			...
			Case cCondPag == "FID" //Programa de fidelidade, Cashback, Crédito Virtual
				cForma := "19"
			Case LjIT240002() .and. cCondPag == "PE" //PIX Estatico
				cForma := "20"
			Case LjIT240002() .and. cCondPag == "CR" //CREDITO LOJA
				cForma := "21"
			//Case LjIT240002() .and. cCondPag == "PNI" //Pagamento Eletrônico não Informado - falha de hardware do sistema emissor
			//	cForma := "22"
...
Return aRet
// --------------------------------------------------------------------------
// Alianca: customizacao da funcao padrao GetNFEImp para quando nao ha integracao com o modulo SigaEIC.
static function _GetNFEIMP (_lParam1, _sDoc, _sSerie, _sFornece, _sLoja, _sTipo_NF, _sPedido, _sItemPC, _sTipoNF, _sProduto, _sItemNF)
	local _aRet     := {}
	local _aAreaAnt := U_ML_SRArea ()
	u_logIni ()
	if _sTipoNF $ "IPC"  // Nota complementar
		u_log ('Sem tratamento para este tipo de nota')
	else
		sd1 -> (dbsetorder (1))  // D1_FILIAL+D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_COD+D1_ITEM
		if sd1 -> (dbseek (xfilial ("SD1") + _sDoc + _sSerie + _sFornece + _sLoja + _sProduto + _sItemNF, .F.))
			u_log ('achei SD1')
	
			// Busca dados nas tabelas de dados complementares da nota, informadas no modulo de livros fiscais.
			cd5 -> (dbsetorder (4))  // CD5_FILIAL+CD5_DOC+CD5_SERIE+CD5_FORNEC+CD5_LOJA+CD5_ITEM
			u_log ('Procurando CD5 com:', xfilial ("CD5") + _sDoc + _sSerie + _sFornece + _sLoja + _sItemNF)
			if cd5 -> (dbseek (xfilial ("CD5") + _sDoc + _sSerie + _sFornece + _sLoja + _sItemNF, .F.))
				u_log ('achei CD5')
				aadd (_aRet, {NIL, NIL, fBuscaCpo ("SB1", 1, xfilial ("SB1") + sd1 -> d1_cod, "B1_POSIPI")})  // ?, ?, NCM
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_NDi})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_DtDi})
				aadd (_aRet, {'I19',  NIL, cd5 -> cd5_NDi})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_DtDi})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_LocDes})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_UfDes})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_DtDes})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_CodExp})
				aadd (_aRet, {NIL, NIL, cvaltochar (val (cd5 -> cd5_NAdic))})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_SqAdic})
				aadd (_aRet, {NIL, NIL, sd1 -> d1_fornece})
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_VDesDI})
				aadd (_aRet, {NIL, NIL, 0})
				aadd (_aRet, {})  //
				aadd (_aRet, {})  //
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_BcImp})  // Base calculo Imp. Importacao
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_DspAd})  // Desp aduaneira
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_VlrII})  // Valor II
				aadd (_aRet, {NIL, NIL, cd5 -> cd5_VlrIOF})  // IOF
				aadd (_aRet, '?')
				aadd (_aRet, '?')
				aadd (_aRet, '?')
				aadd (_aRet, '?')
				...
				aadd (_aRet, '?')
				aadd (_aRet, '?')
				aadd (_aRet, {nil, nil, cd5 -> cd5_acdraw})
				aadd (_aRet, {nil, nil, ''})
				aadd (_aRet, {nil, nil, cd5 -> cd5_vtrans})
				aadd (_aRet, {nil, nil, cd5 -> cd5_vafrmm})
				aadd (_aRet, {nil, nil, cd5 -> cd5_interm})
				aadd (_aRet, {nil, nil, cd5 -> cd5_CNPJae})
				aadd (_aRet, {nil, nil, cd5 -> cd5_UFTerc})
			endif
		endif
	endif
...
// --------------------------------------------------------------------------
// Alianca: Encontra a NF de produtor (para contranotas de uva).
static function _AchaNFPro (_sAliasSD1, _aNFProd)
	local _aAreaAnt := U_ML_SRArea ()
	sa2 -> (dbSetOrder (1))
	if sa2 -> (dbseek (xFilial ("SA2") + (_sAliasSD1) -> D1_FORNECE + (_sAliasSD1) -> D1_LOJA, .F.))
		
		// Nao acrescenta se a NF de produtor jah existe na array.
		if ascan (_aNFProd, {|_aVal| _aVal [2] == sf1 -> f1_vaSePro .and. _aVal [3] == sf1 -> f1_vaNFPro}) == 0
			aadd (_aNFProd, {sf1 -> f1_emissao, ;
			                 sf1 -> f1_vaSePro, ;
			                 sf1 -> f1_vaNFPro, ;
			                 '04', ;  // 04=NF de produtor; 01=NF avulsa
			                 sa2 -> a2_cgc, ;
			                 sa2 -> a2_est, ;
			                 sa2 -> a2_inscr})
		endif
	else
		u_log ('Fornecedor', (_sAliasSD1) -> D1_FORNECE + (_sAliasSD1) -> D1_LOJA, 'nao encontrado!')
	endif
	U_ML_SRArea (_aAreaAnt)
return
...
// Se tiver substituicao e for nota sobre cupom  buscamos o CST da nota.
If lNfCup .And. cCSTrib <> "00" .And. (SM0->M0_ESTCOB $ "CE" .Or. lAuto)
...
If lNfCup .And. !Empty(cD2TesNF) .And. (SM0->M0_ESTCOB $ "CE" .Or. lAuto )
	dbSelectArea("SF4")
	dbSetOrder(1)
	MsSeek(xFilial("SF4")+cD2TesNF)
EndIf        
...
If lNfCup .And. (SM0->M0_ESTCOB $ "CE" .Or. lAuto)
	If Empty(aPis[Len(aPis)]) .And. !empty(SF4->F4_CSTPIS)
		aTail(aPisAlqZ):= {SF4->F4_CSTPIS}		
	EndIf
	If Empty(aCOFINS[Len(aCOFINS)]) .And. !empty(SF4->F4_CSTCOF)
...
static function getIndDeduzDeson(cCST, lDeduzDeson)
	...
	if isNT230004() .and. lDeduzDeson .and. cCST $ '20,30,40,41,50,70,90'
		cString += '<indDeduzDeson>1</indDeduzDeson>'
	endif

return cString

/*/{Protheus.doc} isNT230004()
Funcao temporaria que determina se a NT 2023.004 ja esta em operacao.
Previsao: Homol=11/03/2024 / Prod=01/04/2024
@type function
@version 1.0
@author Felipe Sales Martinez
@since 14/3/2024
@return logical, .T.-> NT em operacao e .F.-> NT ainda nao operante
/*/
static function isNT230004()
if lNT23004 == nil
	lNT23004 := date() >= cTod(superGetMV("MV_NT23004", .F., "01/07/2024"))
endIf
return lNT23004
...
/*/{Protheus.doc} isIT230004()
Funcao temporaria que determina se o Informe Técnico - 2024.002 (v1.00) (Atualização da Tabela de Meios de Pagamento) já esta em operação.
Previsao: Prod=01/07/2024
@type function
@version 1.0
@author Felipe Sales Martinez
@since 12/06/2024
@return logical, 'true' quando Informe Técnico em operacao e 'false' quando NT ainda nao operante
/*/
static function LjIT240002()
return date() >= cToD(SuperGetMV("MV_IT24002",.F.,"01/07/2024")) //TODO: Remover apos prazo da NT em producao: 01/07/2024
